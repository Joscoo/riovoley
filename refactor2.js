...
// Replace tables by finding the exact opening and closing tags, but using more specific regex
// We know each table block is under `filtered.length === 0 ? ... : (`
// CosmeticsTab
content = content.replace(
  /(const CosmeticsTab = \(\) => \{[\s\S]*?\{loading \? \([\s\S]*?\) : filtered\.length === 0 \? \()([\s\S]*?)(\) : \()([\s\S]*?<div className="rounded-xl border border-slate-700\/60 bg-slate-900 shadow-sm">[\s\S]*?<table className="w-full min-w-\[700px\] text-sm">[\s\S]*?<\/div>\n          <Pagination page=\{page\} totalPages=\{totalPages\} total=\{total\} onPage=\{setPage\} \/>\n        <\/div>\n      \)\})/,
  `$1
        <EmptyState
          icon={<FaStore />}
...
// AchievementsTab
content = content.replace(
  /(const AchievementsTab = \(\) => \{[\s\S]*?\{loading \? \([\s\S]*?\) : filtered\.length === 0 \? \()([\s\S]*?)(\) : \()([\s\S]*?<div className="rounded-xl border border-slate-700\/60 bg-slate-900 shadow-sm">[\s\S]*?<table className="w-full min-w-\[600px\] text-sm">[\s\S]*?<\/div>\n          <Pagination page=\{page\} totalPages=\{totalPages\} total=\{total\} onPage=\{setPage\} \/>\n        <\/div>\n      \)\})/,
  `$1
        <EmptyState
          icon={<FaTrophy />}
...
// GoalsTab
content = content.replace(
  /(const GoalsTab = \(\) => \{[\s\S]*?\{loading \? \([\s\S]*?\) : filtered\.length === 0 \? \()([\s\S]*?)(\) : \()([\s\S]*?<div className="rounded-xl border border-slate-700\/60 bg-slate-900 shadow-sm">[\s\S]*?<table className="w-full min-w-\[600px\] text-sm">[\s\S]*?<\/div>\n          <Pagination page=\{page\} totalPages=\{totalPages\} total=\{total\} onPage=\{setPage\} \/>\n        <\/div>\n      \)\})/,
  `$1
        <EmptyState
          icon={<FaBullseye />}
