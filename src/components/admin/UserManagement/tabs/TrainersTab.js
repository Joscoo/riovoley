...
const TrainersTab = ({ userRole }) => {
  ...
  const filteredTrainers = useMemo(() => {
    ...
    if (filters.status === 'active') {
      result = result.filter(t => !t.suspended);
    ...
    if (filters.search) {
      ...
      result = result.filter(t => {
        ...
        return (
          ...
          normalizeText(t.email || '').includes(searchLower)
        );
      });
    }
  ...
  const stats = useMemo(() => {
    ...
    const activos = filteredTrainers.filter(t => !t.suspended).length;
    const suspendidos = filteredTrainers.filter(t => t.suspended).length;
