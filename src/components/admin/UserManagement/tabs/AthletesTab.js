...
const AthletesTab = ({ userRole }) => {
  ...
  const filteredAthletes = useMemo(() => {
    ...
    if (filters.status === 'active') {
      result = result.filter(a => !a.suspended);
    ...
    if (filters.search) {
      ...
      result = result.filter(a => {
        return (
          normalizeText(a.full_name).includes(searchLower) ||
          normalizeText(a.email).includes(searchLower) ||
          normalizeText(a.categoria || '').includes(searchLower)
        );
      });
    }
  ...
  const stats = useMemo(() => {
    ...
    const activos = filteredAthletes.filter(a => !a.suspended).length;
    const suspendidos = filteredAthletes.filter(a => a.suspended).length;
    const byCategory = CATEGORIAS.reduce((acc, cat) => {
      acc[cat] = filteredAthletes.filter(a => a.categoria === cat).length;
      return acc;
    }, {});
